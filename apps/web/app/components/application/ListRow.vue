...
<template>
    <NuxtLink
        ...
        class="group flex flex-col gap-2 rounded-xl border border-jt-line bg-jt-surface p-4 transition hover:bg-jt-surface-hover hover:border-jt-fg-faint"
    >
            ...
            <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-1.5">
                    <span
                        ...
                        class="rounded-md bg-jt-surface-raised px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-jt-fg-faint"
                    >
                        {{ t.common.archived }}
                    </span>
                    <span class="truncate text-base font-semibold text-jt-fg">
                        {{ application.position }}
                    </span>
                </div>
                <p class="mt-0.5 truncate text-sm text-jt-fg-muted">
        ...
        <div class="flex flex-wrap items-center gap-1.5 text-xs text-jt-fg-muted">
            ...
            <span v-if="application.workMode" class="rounded border border-jt-line px-1.5 py-0.5">
                {{ t.workMode[application.workMode] }}
            </span>
            <span v-if="salaryLabel" class="rounded border border-jt-line px-1.5 py-0.5">
                {{ salaryLabel }}
            </span>
            <span
                v-if="openQuestionsOpen > 0"
                class="inline-flex items-center gap-0.5 rounded border border-jt-warning/40 bg-jt-warning-soft px-1.5 py-0.5 text-jt-warning"
                title="Offene Fragen"
            >
            ...
            <span
                v-if="contactCount > 0"
                class="inline-flex items-center gap-0.5 rounded border border-jt-line px-1.5 py-0.5"
                title="Kontakte"
            >
        ...
        <div v-if="techPreview.length > 0" class="flex flex-wrap gap-1">
            <span
                ...
                :key="tech"
                class="rounded bg-jt-brand-soft px-1.5 py-0.5 text-[11px] text-jt-brand"
            >
                {{ tech }}
            </span>
